...
from rl_models import ActorCritic_Agent, make_agent
...
def ppo_main(args, envs, device):
	agent = make_agent(envs, agent_type='actor-critic', device=device)
	adam_epsilon = 1e-5
	...
	for update in training_bar:
		...
		for step in range(0, args.num_steps):
			...
			with torch.no_grad():
				action, log_prob, entropy, value = agent.get_action_and_value(next_obs, next_done)
